...
class preproutines:
    ...
    class to manipulate and prepare Insight data for PIConGPU usage:
        - phase corrections
        - correcting artefacts in the near field
        - propagating
    ...
    def __init__(self, path, name, foc):
        ...
        The far field has to be measured in dependence of "x", "y" and "w". If the scales are named differently, they have to be changed manually in this function.
        ...
        self.wc_idx = int(len(self.w)/2)  # central freq./time index
        
        # fit far field data with gaussian
        # use intensity instead of amplitude to minimize possible halo influence
        popt, pcov = curve_fit(gauss2D, (np.meshgrid(self.y, self.x)), (np.abs(self.Ew[:, :, self.wc_idx])**2).ravel(), p0=(1, 0, 0, .05))
        self.xc = popt[1]
        self.yc = popt[2]
    ...
    def correctPhase(self, GVD=0, TOD=0):
        ...
        For that, we assume perfect compression of the beam center in the near field, thus substracting
        a global phase for every wavelength.
        Furthermore, there is the possibility to add disperion parameters (GVD, TOD) to the data.

        Has to be called after propToNF.
    ...
    def correctUglySpotInNF(self, x_ugly, y_ugly, uglybins = 3, method='linear'):
        ...
        method: RegularGridInterpolator interpolation method, 
                default is 'linear' (since 'cubic' can cause some bumps in the amplitude)
        """
    ...
    def shiftNFtoCenter(self):
        ...
        print("shifted the near field into the center by x = %.2f mm, y = %.2f mm"%(nx*self.dx_NF, ny*self.dy_NF))
    ...
    def toTimeDomain(self, Ew, lamb_supp=8):
        """
        Transform far field from frequency to time domain
        
        Arguments:
        Ew: far field data in frequency domain (at the focus or propagated)
        lamb_supp: the number of sampling points on one wavelength; from this, the number of zeros to be padded will be derived. Default is 8.
        """
        ...
        # set the negative frequency part to 0 (instead of the complex conjugate) and neglegt the imaginary part of the field in the time domain afterwards
    ...
    def saveToOpenPMD(self, outputpath, outputname, energy, pol="x", crop_x=0, crop_y=0, crop_t=0):
        """
        Save the field data in time domain to an OpenPMD checkpoint. This output will be used for the InsightBeam profile.
        ...
        crop_x/y/t: if the field data chunk is too big, one can crop the borders from both sides by the length given with these values (same unit as the corresponding scales). Default is 0.
        """
        ...
        # we just need to store the real part of the field
        ...
        ite = series.iterations[0]  # use just the 0th iteration
        ite.time = 0.0
        ite.dt = self.dt
        ...
        # B_trans = +- E_pol / c
        # B_pol = 0
        # B_z = -+ i/w * d/d_trans E_pol; but neclectable since B_z << B_trans
